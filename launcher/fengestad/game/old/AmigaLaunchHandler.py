...
from fengestad.game.Archive import Archive
...
from .AmigaROMManager import AmigaROMManager

from fengestad.base.I18N import _, ngettext
#from .Util import expand_path

class AmigaLaunchHandler:

    def __init__(self, context, amiga_config):
        self.context = context

        #self.config_name = config_name
        print(repr(self.context.game))
        print(repr(self.context.game.config))
        print(repr(self.context.game.config.copy))
        self.config = amiga_config.copy()
    ...
    def prepare_roms(self):
        ...
        for config_key, src in roms:
            ...
            with open(dest, "wb") as f:
                AmigaROMManager.decrypt_archive_rom(archive, src, file=f)
...
"""
    ...
    def copy_whdload_kickstart(self, base_dir, name, checksums):
        ...
        for checksum in checksums:
            ...
            if path:# and os.path.exists(path):
                ...
                if archive.exists(path):
                    with open(dest, "wb") as f:
                        AmigaROMManager.decrypt_archive_rom(archive, path, file=f)
                    print(repr(dest))
                    break
        else:
    ...
    def get_state_dir(self):
        ...
        return self.context.get_game_state_dir()
    ...
    def create_config(self):
        print("AmigaLaunchHandler.create_config")
        config = {}
    ...
    def unpack_zip(cls, archive, destination):
        try:
            ...
            print(e)
            #raise fs.cause(Exception("Could not unpack game"), e)
            raise Exception("Could not unpack game: {0}".repr(e))

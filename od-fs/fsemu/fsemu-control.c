...
void fsemu_control_set_paused(bool paused) {
    ...
    fsemu_action_post_from_main(paused ? FSEMU_ACTION_PAUSE_ENABLE : FSEMU_ACTION_PAUSE_DISABLE);
}
...
void fsemu_control_set_warp(bool warping) {
    ...
    fsemu_action_post_from_main(warping ? FSEMU_ACTION_WARP_ENABLE : FSEMU_ACTION_WARP_DISABLE);
}

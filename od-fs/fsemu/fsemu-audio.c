...
static void fsemu_audio_update_stats(void) {
    ...
    int inflight = sent_size - dt * consumption;
    if (inflight < 0) {
        fsemu_audio_log_trace("Inflight %d < 0 (consumes %0.1f B/us)\n", inflight, consumption);
    ...
    fsemu_audio_frame_stats_t* stats =
        &fsemu_audio.stats[fsemu_frame_counter_mod(FSEMU_AUDIO_MAX_FRAME_STATS)];
...
#define PID_LATENCY_VALUES 64
    ...
    int latency_avg =
        fsemu_mavgi(&latency_mavg, latency_values, PID_LATENCY_VALUES, fsemu_audio.latency_us);
...
int fsemu_audio_log_buffer_stats(void) {
    ...
    fsemu_audio_log_trace(
        ...
        (int)fsemu_frame_counter(), fsemu_audio_bytes_to_ms(total),
        fsemu_audio_bytes_to_ms(stats->buffer_bytes), fsemu_audio_bytes_to_ms(stats->recent_bytes),
        fsemu_audio_bytes_to_ms(stats->inflight_bytes), (int)stats->dt, consumption);
...
void fsemu_audio_frame_stats(int frame, fsemu_audio_frame_stats_t* stats) {
    memcpy(stats, &fsemu_audio.stats[frame % FSEMU_AUDIO_MAX_FRAME_STATS],
...
void fsemu_audio_update_min_fill(uint8_t volatile* read, uint8_t volatile* write) {
    ...
    if (write >= read) {
        ...
        bytes = (fsemu_audiobuffer.end - read) + (write - fsemu_audiobuffer.data);
    }
...
void fsemu_audio_init(void) {
    ...
    fsemu_audio_log_level = fsemu_option_int_default(FSEMU_OPTION_LOG_AUDIO, FSEMU_LOG_LEVEL_INFO);
    ...
    */
    ...
    int volume = fsemu_option_int_clamped_default(FSEMU_OPTION_VOLUME, 0, 100, 100);
    if (volume == 0) {

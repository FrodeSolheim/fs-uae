...
{
    fsapp_channel_message_t* message
        = FSEMU_UTIL_MALLOC0(fsapp_channel_message_t);
    message->type = type;
...
static PyObject* fsapp_channel_python_create(PyObject* self, PyObject* args)
{
    ...
    PyObject* capsule = PyCapsule_New(
        channel, "fsapp_channel_t", fsapp_channel_python_destructor
    );
    return capsule;
}

static PyObject*
fsapp_channel_python_add_message(PyObject* self, PyObject* args)
{
    PyObject* channel_capsule;
    int type;
    const char* data;
    if (!PyArg_ParseTuple(
            args, "Ois:add_message", &channel_capsule, &type, &data
        )) {
        return NULL;
    }

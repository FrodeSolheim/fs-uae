...
void fs_emu_stat_queue_init(fs_emu_stat_queue* stat_queue, int count) {
    stat_queue->queue = fs_queue_new();
    stat_queue->max_count = count;
    for (int i = 0; i < count; i++) {
        fs_queue_push_head(stat_queue->queue, FS_INT_TO_POINTER(0));
    }
...
void fs_emu_stat_queue_add_entry(fs_emu_stat_queue* stat_queue, int data,
    ...
    if (stat_queue->clear) {
        for (int i = 0; i < stat_queue->max_count; i++) {
            fs_queue_push_head(stat_queue->queue, FS_INT_TO_POINTER(0));
            fs_queue_pop_tail(stat_queue->queue);
        }
        stat_queue->count = 0;
        stat_queue->total = 0;
        stat_queue->clear = 0;
    ...
    */
    ...
    //fs_log("add %d\n", data);
    fs_queue_push_head(stat_queue->queue, FS_INT_TO_POINTER(data));
    int last = FS_POINTER_TO_INT(fs_queue_pop_tail(stat_queue->queue));
    // fixme: min and max

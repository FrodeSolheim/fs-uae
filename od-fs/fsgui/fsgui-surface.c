...
static void
fsgui_surface_actually_create(fsgui_surface_t* surface, int width, int height)
{
    surface->width = width;
...
static PyObject* fsgui_surface_python_create(PyObject* self, PyObject* args)
{
    ...
    if (!PyArg_ParseTuple(args, "(ii)i:create", &width, &height, &special))
        return NULL;

    fsgui_surface_log_debug("Creating %dx%d surface\n", width, height);

    fsgui_surface_t* surface
        = (fsgui_surface_t*)malloc(sizeof(fsgui_surface_t));
    ...
    PyObject* surface_capsule = PyCapsule_New(
        surface, "fsgui_surface_t", fsgui_surface_python_destructor
    );
...
static PyObject* fsgui_surface_python_resize(PyObject* self, PyObject* args)
{
    ...
    int width, height;
    if (!PyArg_ParseTuple(
            args, "O(ii):resize_surface", &surface_capsule, &width, &height
        )) {
        return NULL;
    }
...
static PyObject*
fsgui_surface_python_draw_filled_rectangle(PyObject* self, PyObject* args)
{
    PyObject* surface_capsule;
    SDL_Rect rect;
    int r, g, b, a;
    if (!PyArg_ParseTuple(
            args, "O(ii)(ii)(iiii):draw_filled_rectangle", &surface_capsule,
            &rect.x, &rect.y, &rect.w, &rect.h, &r, &g, &b, &a
        )) {
        return NULL;
    }
...
static PyObject* fsgui_surface_python_draw_line(PyObject* self, PyObject* args)
{
    ...
    int r, g, b, a;
    if (!PyArg_ParseTuple(
            args, "O(ii)(ii)(iiii):draw_line", &surface_capsule, &x1, &y1, &x2,
            &y2, &r, &g, &b, &a
        )) {
        return NULL;
    }
...
static PyMethodDef fsgui_surface_python_methods[]
    = { { "create", fsgui_surface_python_create, METH_VARARGS, "..." },
        ...
        { "draw_filled_rectangle", fsgui_surface_python_draw_filled_rectangle,
          METH_VARARGS, "..." },
        { "draw_line", fsgui_surface_python_draw_line, METH_VARARGS, "..." },
        { NULL, NULL, 0, NULL } };

static PyModuleDef fsgui_surface_python_module

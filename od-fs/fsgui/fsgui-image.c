...
static void fsgui_image_init_9patch(fsgui_image_t* image)
{
    ...
    fsgui_image_log_debug(
        "9 patch image border left %d top %d right %d bottom %d\n",
...
fsgui_image_t* fsgui_image_load(const char* name)
{
    ...
    if (surface->format == SDL_PIXELFORMAT_RGBA32) {
        ...
        SDL_LogError(
            SDL_LOG_CATEGORY_ERROR, "Pixel format (0x%x) is not RGBA32",
            surface->format
        );
...
static void fsgui_image_python_destructor(PyObject* image_capsule)
{
    ...
    fsgui_image_log_warning(
        "fsgui_image_python_destructor FIXME: Not implemented\n"
    );
#endif
}
...
static PyObject* fsgui_image_python_load(PyObject* self, PyObject* args)
{
    ...
    if (image == NULL) {
        SDL_LogError(
            SDL_LOG_CATEGORY_ERROR, "Could not load image %s: %s", name,
            SDL_GetError()
        );
        return NULL;
    }
    ...
    PyObject* capsule
        = PyCapsule_New(image, "fsgui_image_t", fsgui_image_python_destructor);
    return capsule;
}
...
static PyObject* fsgui_image_python_get_size(PyObject* self, PyObject* args)
{
    ...
    fsgui_image_t* image
        = (fsgui_image_t*)PyCapsule_GetPointer(image_capsule, "fsgui_image_t");
    SDL_assert(image);
...
static PyObject* fsgui_image_python_draw(PyObject* self, PyObject* args)
{
    ...
    int x, y;
    if (!PyArg_ParseTuple(
            args, "OO(ii):draw", &image_capsule, &surface_capsule, &x, &y
        )) {
        return NULL;
    }
    fsgui_image_t* image
...
static PyObject*
fsgui_image_python_draw_stretched(PyObject* self, PyObject* args)
{
    PyObject *image_capsule, *surface_capsule;
    int x, y, w, h;
    if (!PyArg_ParseTuple(
            args, "OO(ii)(ii):draw_stretched", &image_capsule,
            &surface_capsule, &x, &y, &w, &h
        )) {
        return NULL;
    }
    fsgui_image_t* image
    ...
    if (image->left) {
        SDL_Rect src_rect = { .x = 1,
                              .y = 1,
                              .w = image->surface->w - 2,
                              .h = image->surface->h - 2 };
        SDL_BlitSurface9Grid(
            image->surface, &src_rect, image->left, image->right, image->top,
            image->bottom, 1.0, SDL_SCALEMODE_LINEAR, surface->sdl, &dst_rect
        );

    } else {
        SDL_BlitSurfaceScaled(
            image->surface, NULL, surface->sdl, &dst_rect, SDL_SCALEMODE_LINEAR
        );
    }

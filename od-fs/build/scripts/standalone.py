...
def fix_linux_binary(path: str) -> int:
    changes = 0
...
def linux_iteration(app: str) -> int:
    binaries: List[str] = []
    binaries_dir = app
...
def fix_macos_binary_2(path: str, frameworks_dir: str) -> int:
    print("fixing", path)
    changes = 0
    if not os.path.exists(path):
        raise Exception("could not find " + repr(path))
...
def macos_iteration_2(app: str) -> int:
    binaries: List[str] = []
...
def fix_windows_binary(path: str, app_dir: str) -> int:
    ...
    for line in data.split("\n"):
        ...
        if True:
            ...
            if not os.path.exists(src):
                ...
                print("Checking", src)
            print(src)
            included_libraries.setdefault(src, set()).add(fix_dll_name)
...
def windows_iteration(app: str) -> int:
    binaries: List[str] = []

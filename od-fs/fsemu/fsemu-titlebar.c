...
void fsemu_titlebar_update(void) {
    ...
    if (!fsemu_titlebar.title_item.image && ui_scale > 0) {
        ...
        // FIXME: Semi-bold ?
        fsemu_font_t* font = fsemu_fontcache_font("SairaCondensed-SemiBold.ttf", 19 * ui_scale);
        ...
        fsemu_titlebar.title_image =
            fsemu_font_render_text_to_image(font, title_upper, fsemu_titlebar.title_color);
        free(title_upper);
#if 0
        ...
        fsemu_gui_image(item,
                        // 20 * ui_scale
                        fsemu_titlebar_height() + FSEMU_TITLEBAR_MENU_SPACING * ui_scale, 0,
                        fsemu_titlebar.title_image->width, fsemu_titlebar.title_image->height,
                        fsemu_titlebar.title_image);
        item->z_index = 9001;
    ...
    uint32_t opacity_color =
        FSEMU_COLOR_RGB_A(0xffffff, window_is_active ? fsemu_titlebar.title_color
    ...
    if (fsemu_titlebar.title_image) {
        int title_y = (fsemu_titlebar.height - fsemu_titlebar.title_image->height) / 2;
        // Hand-tuned y offset for common ui scales for better text centering.
        if (ui_scale == 1.0) {
    ...
    fsemu_gui_item_set_visible(&fsemu_titlebar.background_item, is_visible && is_static);
    fsemu_titlebar.background_item.color = window_is_active
                                               ? fsemu_titlebar.background_color
                                               : fsemu_titlebar.background_color_inactive;

    fsemu_gui_item_set_visible(&fsemu_titlebar.background_ws_item, is_visible && !is_static);
    ...
    fsemu_titlebar.maximize_bg_item.color = active == FSEMU_TITLEBAR_MAXIMIZE
    ...
    fsemu_titlebar.menu_bg_item.color =
        active == FSEMU_TITLEBAR_MENU ? FSEMU_TITLEBAR_MENU_ACTIVE : FSEMU_TITLEBAR_MENU_HOVER;

    fsemu_gui_item_set_visible(&fsemu_titlebar.minimize_bg_item,
                               is_visible && hover == FSEMU_TITLEBAR_MINIMIZE);
    fsemu_titlebar.minimize_bg_item.color = active == FSEMU_TITLEBAR_MINIMIZE
    ...
    fsemu_gui_item_set_visible(&fsemu_titlebar.close_bg_item,
                               is_visible && hover == FSEMU_TITLEBAR_CLOSE);
    fsemu_titlebar.close_bg_item.color =
        active == FSEMU_TITLEBAR_CLOSE ? FSEMU_TITLEBAR_CLOSE_ACTIVE : FSEMU_TITLEBAR_CLOSE_HOVER;
#endif
}
...
bool fsemu_titlebar_handle_mouse(fsemu_mouse_event_t* event) {
    ...
    if ((visible && event->y < fsemu_titlebar.height) || fsemu_titlebar.mouse_trapped) {
...
static void fsemu_titlebar_init_widgets(void) {
    ...
    fsemu_gui_rectangle(item, 0, 0, 0, fsemu_titlebar.height, fsemu_titlebar.background_color);
    item->z_index = 9000;
    fsemu_gui_add_item(item);

    // With shadow
    item = &fsemu_titlebar.background_ws_item;
    fsemu_titlebar.background_ws_image = fsemu_image_load("TitleBarWithShadow.png");
    fsemu_gui_image(item, 0, 0, 1000, fsemu_titlebar.background_ws_image->height,
    ...
    fsemu_gui_image(item, 0, 0, fsemu_titlebar.menu_icon->width, fsemu_titlebar.menu_icon->height,
    ...
    fsemu_titlebar.minimize_icon = fsemu_image_load("TitleBarMinimizeIcon.png");
    fsemu_gui_image(item, 0, 0, fsemu_titlebar.minimize_icon->width,
                    fsemu_titlebar.minimize_icon->height, fsemu_titlebar.minimize_icon);
    item->z_index = 9002;
    fsemu_gui_add_item(item);
    ...
    fsemu_titlebar.maximize_icon = fsemu_image_load("TitleBarMaximizeIcon.png");
    fsemu_gui_image(item, 0, 0, fsemu_titlebar.maximize_icon->width,
                    fsemu_titlebar.maximize_icon->height, fsemu_titlebar.maximize_icon);
    item->z_index = 9002;
    fsemu_gui_add_item(item);
    ...
    fsemu_titlebar.close_icon = fsemu_image_load("TitleBarCloseIcon.png");
    fsemu_gui_image(item, 0, 0, fsemu_titlebar.close_icon->width, fsemu_titlebar.close_icon->height,
...
void fsemu_titlebar_init(void) {
    ...
    fsemu_option_read_bool_default(FSEMU_OPTION_SYSTEM_TITLEBAR, &fsemu_titlebar.use_system, false);
    ...
    fsemu_titlebar.background_color_inactive = FSEMU_TITLEBAR_BACKGROUND_INACTIVE;

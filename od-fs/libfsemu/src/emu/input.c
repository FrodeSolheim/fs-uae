                            ...
                            int middle, int right, int wheel_axis)
{
    ...
    for (int i = 0; i < FS_ML_INPUT_DEVICES_MAX; i++)
    {
        ...
        if (device.name == NULL || ((g_ascii_strcasecmp(device.name, name) != 0) &&
...
static void map_custom_keyboard_actions(void)
{
    ...
    while (*key)
    {
        ...
        map_custom_key_action(keyl, k, "ctrl_alt_", FS_ML_KEY_MOD_CTRL | FS_ML_KEY_MOD_ALT);
        map_custom_key_action(keyl, k, "shift_", FS_ML_KEY_MOD_SHIFT);
        map_custom_key_action(keyl, k, "shift_alt_", FS_ML_KEY_MOD_SHIFT | FS_ML_KEY_MOD_ALT);
...
#define PROCESS_STATE                                                                                                              \
    ...
    {                                                                                                                              \
        ...
        if (state && (menu_action == ACTION_MENU_START || menu_action == ACTION_MENU_SELECT || menu_action == ACTION_MENU_MENU) && \
    ...
    while ((name = g_dir_read_name(dir)) != NULL)
    {
        ...
        while ((name2 = g_dir_read_name(dir2)) != NULL)
        {
            ...
            g_hash_table_insert(g_input_config_paths, g_strdup(full_name),
                                g_strdup(path));
            g_hash_table_insert(g_input_config_paths, g_strconcat(full_name, "_usb", NULL), g_strdup(path));
            ////fs_log("[*] %s\n", full_name);
            g_hash_table_insert(g_input_config_paths, g_strdup(name3),
                                g_strdup(path));
            g_hash_table_insert(g_input_config_paths, g_strconcat(name3, "_usb", NULL), g_strdup(path));
            ////fs_log("[*] %s\n", name3);
            g_free(full_name);
            g_free(path);
            g_free(name3);
...
static int input_function(fs_ml_event *event)
{
    ...
    */
    else if (event->type == FS_ML_MOUSEBUTTONDOWN || event->type == FS_ML_MOUSEBUTTONUP)
    {
        if (event->type == FS_ML_MOUSEBUTTONDOWN && event->button.device == fs_ml_first_mouse_index())
        {
            ...
            if (fs_emu_input_grab())
            {
                if (g_middle_click_ungrab && event->button.button == FS_ML_BUTTON_MIDDLE)
                {

...
void uae_blitter_save_extended_state(uae_savestate_context_t *ctx)
{
	...
	sr_int(bltptxc);
	sr_int(bltptxpos);
	sr_enum(bltstate, blitter_states);
	...
	int blit_diag_type = get_cycle_diagram_type(blit_diag);
	sr_int(blit_diag_type);
	if (ctx->load) {
		blit_diag = set_cycle_diagram_type(blit_diag_type);
	}

	// if (ctx->load) {
	// 	get_cycle_diagram_type (blit_diag)
	// } else {
	// 	int blit_diag_type = get_cycle_diagram_typ (blit_diag);
	// 	sr_int(blit_diag_type);
	// }

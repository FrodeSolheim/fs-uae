...
void fsemu_titlebar_update(void)
{
    ...
    if (is_static) {
        ...
        if (fsemu_titlebar.background_item.rect.y > -fsemu_titlebar.height) {
            // y = fsemu_titlebar.background_item.rect.y - 4;
            // fsemu_titlebar.background.rect.y -= 4;
            // fsemu_titlebar.background_item.rect.y = -fsemu_titlebar.height;
            y = -fsemu_titlebar.height;
            // fsemu_titlebar.background_item.dirty = true;
            // fsemu_titlebar.close_bg_item.rect.y = -fsemu_titlebar.height;
        }
    }

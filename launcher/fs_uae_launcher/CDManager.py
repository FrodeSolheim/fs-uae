...
class CDManager:
    ...
    @classmethod
    def eject(cls, drive):
        Config.set("cdrom_drive_{0}".format(drive), "")
        Config.set("x_cdrom_drive_{0}_sha1".format(drive), "")
        #Config.set("x_cdrom_drive_{0}_name".format(drive), "")

    @classmethod
    def clear_cdrom_list(cls):
        for i in range(20):
            Config.set("cdrom_image_{0}".format(i), "")
            #Config.set("x_cdrom_image_{0}_name".format(i), "")
            Config.set("x_cdrom_image_{0}_sha1".format(i), "")
    ...
    def multiselect(cls, parent=None):
        ...
        for i, path in enumerate(paths):
            ...
            if i < 1:
                Config.set("cdrom_drive_{0}".format(i), path)
                #Config.set("x_cdrom_drive_{0}_sha1".format(i), sha1)
                Config.set("x_cdrom_drive_{0}_name".format(i), file)
            Config.set("cdrom_image_{0}".format(i), path)
            #Config.set("x_cdrom_image_{0}_sha1".format(i), sha1)
            Config.set("x_cdrom_image_{0}_name".format(i), file)
        # blank the rest of the drives
        for i in range(len(paths), 1):
            Config.set("cdrom_drive_{0}".format(i), "")

...
class UltimateListLineData(object):
    ...
    def DrawInReportMode(self, dc, line, rect, rectHL, highlighted, current, enabled, oldPN, oldBR):
        ...
        # FIXME: HACK to make the line fill out completely (increase width)
        #rect[2] += 100
        #rectHL[2] += 100
...
##                    width -= IMAGE_MARGIN_IN_REPORT_MODE - MARGIN_BETWEEN_TEXT_AND_ICON
            ...
            if item.HasText():
                ...
                #DASH = u"("
                SPLIT = u"\n"
                primary = item.GetText()
                secondary = ""
                if SPLIT in primary:
                    #primary, secondary = primary.split(DASH, 1)
                ...
                dw = self.DrawTextFormatted(dc, primary, line, col, itemRect, overflow)
                ...
                if secondary:
                    ...
                    #gc.DrawText(secondary, itemRect[0], itemRect[1])
                    #dc.SetTextForeground(wx.Colour(0x0, 0x0, 0x0, 0x80))
                    dc.DrawLabel(secondary, itemRect, wx.ALIGN_CENTER_VERTICAL)
                    coloured = True

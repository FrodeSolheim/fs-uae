...
static void fsemu_inputdevice_python_destructor(PyObject* object) {
    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                "fsemu_inputdevice_python_destructor IS NOT IMPLEMENTED");
}

static PyObject* fsemu_inputdevice_python_create(PyObject* self, PyObject* args) {
    if (!PyArg_ParseTuple(args, ":create")) {
        return NULL;
    }
    fsemu_inputdevice_t* device = fsemu_inputdevice_new();
    PyObject* capsule =
        PyCapsule_New(device, "fsemu_inputdevice_t", fsemu_inputdevice_python_destructor);
    return capsule;
}

static PyObject* fsemu_inputdevice_python_set_name(PyObject* self, PyObject* args) {
    PyObject* capsule;
    const char* name;
...
static PyObject* fsemu_inputdevice_python_set_type(PyObject* self, PyObject* args) {
    PyObject* capsule;
    int type;
...
static PyModuleDef fsemu_inputdevice_python_module = {PyModuleDef_HEAD_INIT,
                                                      "fsemu_inputdevice",
                                                      NULL,
                                                      -1,
...
void fsemu_inputdevice_init_module(void) {
    ...
    PyImport_AppendInittab("fsemu_inputdevice", &fsemu_inputdevice_python_init);
}

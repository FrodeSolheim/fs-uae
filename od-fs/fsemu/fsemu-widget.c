...
void fsemu_widget_ref(fsemu_widget_t* widget) {
    fslib_refable_ref(widget);
}

void fsemu_widget_unref(fsemu_widget_t* widget) {
    fslib_refable_unref(widget);
}
...
fsemu_widget_t* fsemu_widget_new(void) {
    ...
    fslib_refable_init_with_finalizer(widget, fsemu_widget_finalize);
    return widget;
}
...
void fsemu_widget_remove_child(fsemu_widget_t* widget, fsemu_widget_t* remove_child) {
    fsemu_widget_t* child = widget->first_child;
...
void fsemu_widget_set_image(fsemu_widget_t* widget, fsemu_image_t* image, bool image_owner) {
    if (widget->image) {
        fslib_refable_unref(widget->image);
    }
    widget->image = image;
    if (widget->image) {
        if (!image_owner) {
            fslib_refable_ref(widget->image);
        }
    }
...
void fsemu_widget_set_coords(fsemu_widget_t* widget, double x, double y, double w, double h,
                             int coords) {
...
void fsemu_widget_set_top_2(fsemu_widget_t* widget, double offset_top, int anchor_top) {
    widget->top.offset = offset_top;
    widget->top.anchor = anchor_top;
}

void fsemu_widget_set_right_2(fsemu_widget_t* widget, double offset_right, int anchor_right) {
    widget->right.offset = offset_right;
    widget->right.anchor = anchor_right;
}

void fsemu_widget_set_bottom_2(fsemu_widget_t* widget, double offset_bottom, int anchor_bottom) {
    widget->bottom.offset = offset_bottom;
    widget->bottom.anchor = anchor_bottom;
}

void fsemu_widget_set_left_2(fsemu_widget_t* widget, double offset_left, int anchor_left) {
    widget->left.offset = offset_left;
...
void fsemu_widget_set_coordinates(fsemu_widget_t* widget, int anchor_left, double offset_left,
                                  int anchor_top, double offset_top, int anchor_right,
                                  double offset_right, int anchor_bottom, double offset_bottom) {
    widget->left.anchor = anchor_left;
    widget->left.offset = offset_left;
    widget->top.anchor = anchor_top;
    widget->top.offset = offset_top;
...
void fsemu_widget_set_coordinates_2(fsemu_widget_t* widget, int anchor_top, double offset_top,
                                    int anchor_left, double offset_left, int anchor_bottom,
                                    double offset_bottom, int anchor_right, double offset_right) {
    widget->top.anchor = anchor_top;
    widget->top.offset = offset_top;
    widget->left.anchor = anchor_left;
    widget->left.offset = offset_left;
...
void fsemu_widget_set_text_color(fsemu_widget_t* widget, fsemu_color_t text_color) {
...
void fsemu_widget_set_text_transform(fsemu_widget_t* widget, int text_transform) {
...
void fsemu_widget_update_text_image(fsemu_widget_t* widget) {
    ...
    fsemu_font_t* font = fsemu_fontcache_font("SairaCondensed-SemiBold.ttf", widget->font_size);
    fsemu_image_t* textimage = fsemu_font_render_text_to_image(font, text, widget->text_color);
    fsemu_widget_log_debug("textimage size %dx%d for text %s\n", textimage->width,
                           textimage->height, text);
    if (widget->textimage) {
        fsemu_image_unref(widget->textimage);
    }

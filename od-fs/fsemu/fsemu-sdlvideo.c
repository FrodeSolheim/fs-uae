...
void fsemu_sdlvideo_init(void)
{
    ...
    // fsemu_sdlvideo.renderer = SDL_CreateRenderer(window, NULL);
    // fsemu_video_log("%p\n", window);
...
void fsemu_sdlvideo_render(void)
{
    ...
    // SDL_Texture *texture = SDL_CreateTexture(fsemu_sdlvideo.renderer,
    //                                          SDL_PIXELFORMAT_RGBA8888,
    //                                          SDL_TEXTUREACCESS_TARGET,
    //                                          692,
    //                                          540);

    // SDL_RenderTexture(fsemu_sdlvideo.renderer, texture, NULL, &dst);

    SDL_SetRenderDrawColor(fsemu_sdlvideo.renderer, 0xFF, 0x00, 0x00, 0xFF);
    SDL_RenderRect(fsemu_sdlvideo.renderer, &dst);
